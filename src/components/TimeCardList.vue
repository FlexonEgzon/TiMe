...
const currCard = computed(() => {
  //Get Card that has been clicked for description/details
  return activeCardId.value === null
    ? undefined
    : store.cards.find((x) => x.id === activeCardId.value);
});
...
function saveDetails(payload: TimeCardDetailsPayload) {
